"""
Cross-Layer Gate Coherence Across Training (TinyLlama-1.1B)
============================================================

TinyLlama port of experiment_gate_crosslayer_olmo.py.
...
Critical question: OLMo's Jaccard data showed the inversion clearly at
late training: early-layer pairs had low Jaccard (different dimensions
open at adjacent layers), final-layer pairs had high Jaccard (same
dimensions open). If TinyLlama reproduces this pattern at its available
checkpoints, the phenomenon holds across architectures.
...
Also computes non-adjacent pairs (first-last, first-mid, mid-last) plus
quarter-boundary pairs to capture depth scaling effects in the 22-layer
architecture.

Architecture note:
    TinyLlama uses the Llama 2 architecture with SwiGLU (gate_proj,
    up_proj, down_proj), identical MLP structure to OLMo-2. Key
    differences from OLMo-2-1B:
...
# Use the final checkpoint repo for the tokenizer
...
def get_tokenizer():
    ...
    return AutoTokenizer.from_pretrained(TOKENIZER_REPO, cache_dir=CACHE_DIR)
...
def run_experiment(n_samples: int, output_dir: str):
    ...
    for step, tokens, repo_id in CHECKPOINT_REPOS:
        ...
        # Non-adjacent pairs (expanded for 22-layer architecture)
        global_pairs = [
        ...
        # Summary: mean adjacent cosine and jaccard
        adj_cosines = [
            step_results["adjacent_pairs"][f"{li}_{li+1}"]["cosine_similarity"]["mean"]
            for li in range(N_LAYERS - 1)
        ]
        adj_jaccards = [
            step_results["adjacent_pairs"][f"{li}_{li+1}"]["jaccard_similarity"]["mean"]
            for li in range(N_LAYERS - 1)
        ]
        step_results["summary"] = {
            ...
            "mean_adjacent_jaccard": float(np.mean(adj_jaccards)),
            "std_adjacent_jaccard": float(np.std(adj_jaccards)),
            "min_adjacent_jaccard": float(np.min(adj_jaccards)),
            "max_adjacent_jaccard": float(np.max(adj_jaccards)),
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cross-Layer Gate Coherence (TinyLlama-1.1B)")
