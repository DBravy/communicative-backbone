"""
Gate Selectivity Across Training (TinyLlama-1.1B)
==================================================

TinyLlama port of experiment_gate_selectivity_olmo.py (originally for OLMo-2-1B).
...
Critical question: Does TinyLlama reproduce OLMo's depth gradient in gate
sparsity at late training (high sparsity at early layers, low sparsity at
final layers)? If so, the SwiGLU sparsity inversion is a general
phenomenon rather than an OLMo-specific one.

Architecture note:
    TinyLlama uses the Llama 2 architecture with SwiGLU (gate_proj,
    up_proj, down_proj), identical MLP structure to OLMo. Key differences:
      - 22 layers (vs OLMo's 16)
      - d_ff = 5632 (vs OLMo's 8192)
      - Grouped Query Attention (4 KV heads, 32 Q heads)
    The gate analysis is structurally identical since both use SwiGLU.

Models: TinyLlama-1.1B
Checkpoints: 50K, 240K, 480K, 715K, 955K, 1195K, 1431K steps
    (published as separate HuggingFace repos, not revisions)

Usage:
    python experiment_gate_selectivity_tinyllama.py [--n_samples 256]
"""
...
# Checkpoints are stored as separate HF repos, not as revisions.
# Each entry: (step_count, tokens_seen, repo_id)
CHECKPOINT_REPOS = [
...
def run_experiment(n_samples: int, output_dir: str):
    ...
    out_path = os.path.join(output_dir, "gate_selectivity_tinyllama_1b.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Gate Selectivity Across Training (TinyLlama-1.1B)")
