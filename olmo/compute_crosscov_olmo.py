"""
Compute per-layer MLP cross-covariance spectrum for OLMo-2-1B.
===============================================================
...
Designed to run on a MacBook at float16. No backprop required.

Usage:
    python compute_crosscov_olmo.py [--n_tokens 500] [--device mps]
"""
...
# Sample texts for collecting activations. Using varied content
# to get representative statistics.
SAMPLE_TEXTS = [
...
def load_model(device, dtype):
    """Load the final checkpoint of OLMo-2-1B."""
    print(f"Loading OLMo-2-0425-1B (final checkpoint) on {device} at {dtype}...")
...
def collect_mlp_io(model, tokenizer, texts, device, n_tokens, n_layers):
    ...
    """
    # Storage: accumulate in float32 on CPU for numerical stability
    ...
    # Concatenate and trim to n_tokens
    result = {}
    for i in range(n_layers):
...
def compute_crosscov_spectrum(inputs, outputs):
    ...
    # Cross-covariance: (d_model x d_model)
    ...
    # Also return top singular values for inspection
    top_sv = S[:20].tolist()

    return eff_rank, total_var, top_sv


def run(n_tokens, device, dtype):
    n_layers = 16
    ...
    out_path = os.path.join(RESULTS_DIR, "crosscov_olmo_1b_final.json")
    ...
    print("=" * 60)
    print(f"\nPrediction: effective rank should peak around layers 2-5")
    print(f"Block boundary from pairwise heatmap: boundaries 2-3, 3-4, 4-5")
...
if __name__ == "__main__":
    ...
    run(args.n_tokens, device, dtype)
